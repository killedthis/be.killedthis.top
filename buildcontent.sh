...
wwwroot="/home/www/sites"
projectroot="${wwwroot}/killedthis.top"
...
for SERVICE in "${SERVICES[@]}"; do
	...
	for INDEX in "${!TITLES[@]}"; do
		if [[ ${TMDBID[${INDEX}]} == "NULL" ]]; then
			...
			(
				...
				echo "${TITLES[${INDEX}]}"
				/home/www/golang/tmdb/tmdb -s "${TITLES[${INDEX}]}"
			)
		else
			...
			(
				cd /home/www/golang/tmdb/ || exit
				echo "${TMDBID[${INDEX}]}"
				/home/www/golang/tmdb/tmdb -t="${TMDBID[${INDEX}]}"
			)
		fi
	...
	cp "${projectroot}"/site.template "${projectroot}"/"${SERVICE,}".killedthis.top.html

	#insert content into template
	sed -i "s|{template}|${TITLELIST}|g" "${projectroot}/${SERVICE,}.killedthis.top.html"
	sed -i "s|{year_filter}|${YEARFILTER}|g" "${projectroot}/${SERVICE,}.killedthis.top.html"
	sed -i "s|{month_filter}|${MONTHFILTER}|g" "${projectroot}/${SERVICE,}.killedthis.top.html"

	#replace website title with service provider name
	sed -i "s|{website_title}|${SERVICE} killed the following shows|g" "${projectroot}/${SERVICE,}.killedthis.top.html"
done

cp "${projectroot}"/root.template "${projectroot}"/root.html
cp "${projectroot}"/menu.template "${projectroot}"/menu.js
sed -i "s|{menu}|${MENUSTR}|g" "${projectroot}/menu.js"
sed -i "s/{website_title}/KilledThis/g" "${projectroot}/root.html"
